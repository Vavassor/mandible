...
static void open_stream(StreamManager *stream_manager, const char *filename,
	...
	if (!decoder || open_error) {
		LOG_ERROR("test music %s failed to load: %i", filename, open_error);
	}
...
static void decode_streams(StreamManager *stream_manager, int frames,
	...
	FOR_N(i, stream_manager->stream_count) {
		Stream *stream = stream_manager->streams + i;
		int frames_decoded =
			stb_vorbis_get_samples_float_interleaved(stream->decoder,
													 channels,
...
static void *run_mixer_thread(void *argument) {
	...
	open_stream(&system->stream_manager,
				"Assets/06_Out_With_My_Girlfriends.ogg", samples);

	system->stream_manager.streams[0].volume = 0.0f;

	const char *test_filename = "Assets/grass.wav";
	system->test_wave = wave_open_file(test_filename);
	if (!system->test_wave) {
		LOG_ERROR("Failed to open file %s.", test_filename);
	}
	...
	while (atomic_flag_test_and_set(&system->quit)) {
		...
		fill_with_silence(system->mixed_samples, specification.silence, samples);
		wave_decode_interleaved(system->test_wave, system->specification.channels,
								system->mixed_samples, samples);
	...
	wave_close_file(system->test_wave);
